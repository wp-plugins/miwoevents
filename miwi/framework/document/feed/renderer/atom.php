...
/**
 * MDocumentRenderer_Atom is a feed that implements the atom specification
 *
 * Please note that Must by using this class you won't automatically
 ...
 * @since       11.1
 */
class MDocumentRendererAtom extends MDocumentRenderer
{
	/**
	 * Document mime type
	 *
	...
	public function render($name = '', $params = null, $content = null)
	{
		...
		$syndicationURL = MRoute::_('&format=feed&type=atom');

		if ($app->getCfg('sitename_pagetitles', 0) == 1)
		{
			$title = MText::sprintf('MPAGETITLE', $app->getCfg('sitename'), $data->title);
		}
		elseif ($app->getCfg('sitename_pagetitles', 0) == 2)
		{
			$title = MText::sprintf('MPAGETITLE', $data->title, $app->getCfg('sitename'));
		}
		else
		{
			$title = $data->title;
		...
		$feed .= "	<generator uri=\"http://Moomla.org\" version=\"1.6\">" . $data->getGenerator() . "</generator>\n";
		...
		for ($i = 0, $count = count($data->items); $i < $count; $i++)
		{
			...
			$itemDate = MFactory::getDate($data->items[$i]->date);
